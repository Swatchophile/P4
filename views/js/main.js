...
var resizePizzas = function(size) { 
	...
	function sizeSwitcher (size) {
		/*
		Get the pizzaSize div element and store it in a variable outside 
		the switch statement so it's not recalculated every time.
		Also, combine the two switch statements (in the original file) into one
		*/
	...
	// dx Returns the size difference to change a pizza element from one size to another. Called by changePizzaSlices(size).
	...
	function changePizzaSizes() {
		...
		var oldwidth = pizzaContainers[0].offsetWidth;
...
function updatePositions() {
	...
	//Creating a variable to compute and store the value of document.body.scrollTop / 1250 outside the for loop so it's not recalculated at every iteration
	var scrollVal = document.body.scrollTop / 1250;
	for (var i = 0; i < items.length; i++) {
		var phase = Math.sin(scrollVal + (i % 5));
		items[i].style.left = items[i].basicLeft + 100 * phase + 'px';
		//using translate3d to reduce paint time
...
document.addEventListener('DOMContentLoaded', function() {
	...
	//reducing the number of pizzas to 120
	for (var i = 0; i < 120; i++) {
		var elem = document.createElement('img');
		elem.className = 'mover';
		//--commenting out the following and moving the fixed style properties of the mover class to the stylesheet. See pizza.html--
